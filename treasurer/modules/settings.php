<?php 
// Settings fragment for Treasurer portal 
...
.settings-section { background:var(--surface); border:1px solid var(--border); border-radius:var(--radius); padding:20px; box-shadow:var(--shadow-sm); }
.settings-section h4 { font-size: 15px; font-weight: 700; color: var(--text); margin-bottom: 16px; }
...
.btn-gold { background: var(--gold); color: #fff; border: none; padding: 10px 18px; border-radius: 8px; cursor: pointer; font-size: 13px; font-weight: 600; transition: all var(--transition); display: inline-flex; align-items: center; gap: 6px; }
.btn-gold:hover { background: var(--gold-light); }
.btn-danger { background: #c62828; color: #fff; border: none; padding: 8px 14px; border-radius: 8px; cursor: pointer; font-size: 12px; font-weight: 600; transition: all var(--transition); display: inline-flex; align-items: center; gap: 6px; white-space: nowrap; min-width: max-content; justify-content: center; }
.btn-danger:hover { background: #a61f1f; }
.two-factor-actions { display: flex; flex-wrap: wrap; gap: 8px; }
.two-factor-actions .btn-outline { flex: 1 1 180px !important; justify-content: center; }
.two-factor-actions .btn-danger { flex: 0 0 auto; }
...
<div class="page-header">
  ...
  <p>Configuration & account management</p>
</div>

<div class="settings-grid" style="margin-bottom:16px;">
  <div class="settings-section">
    <h4>👤 Account Details</h4>
    <div class="form-group"><label class="form-label">Username</label><input class="form-control" type="text" id="settingsUsername" value="<?php echo htmlspecialchars($_SESSION['username'] ?? 'treasurer', ENT_QUOTES, 'UTF-8'); ?>" placeholder="Enter username"></div>
    <div class="form-group"><label class="form-label">Current Password</label><div style="position:relative;"><input class="form-control" type="password" id="settingsCurrentPassword" placeholder="Enter current password" style="padding-right:36px;"><button type="button" onclick="toggleSettingsPw('settingsCurrentPassword','spIcon1')" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#6b7280;font-size:1rem;padding:0;"><i class="bi bi-eye-slash" id="spIcon1"></i></button></div></div>
    <div class="form-group"><label class="form-label">New Password</label><div style="position:relative;"><input class="form-control" type="password" id="settingsNewPassword" placeholder="Enter new password" style="padding-right:36px;"><button type="button" onclick="toggleSettingsPw('settingsNewPassword','spIcon2')" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#6b7280;font-size:1rem;padding:0;"><i class="bi bi-eye-slash" id="spIcon2"></i></button></div></div>
    ...
    <button class="btn-save" onclick="updateAccountDetails()">Update Account Details</button>
    <div id="accountDetailsMessage" class="small mt-2" style="font-size: 12.5px; margin-top: 8px;"></div>
    ...
    <div style="margin-top: 24px;">
      ...
      <!-- Setup Panel -->
      <div id="twoFactorSetup" style="display: none; background: var(--cream); border: 1px solid rgba(200,168,75,0.2); border-radius: 8px; padding: 16px; margin-top: 12px;">
        <div style="font-size: 13px; font-weight: 600; color: var(--text); margin-bottom: 12px;">✉️ Setup Email Two-Factor Authentication</div>

        <div class="settings-light-panel" style="border-radius: 8px; padding: 16px; margin-bottom: 12px;">
          ...
          <div class="form-group">
            ...
            <div style="display: flex; gap: 8px;">
              <input class="form-control" id="twoFactorEmail" type="email" placeholder="yourname@gmail.com" style="flex: 1;">
              <button class="btn-save btn-sm" onclick="sendEmailOTP()" style="white-space: nowrap; width: auto;">✉ Send OTP</button>
            </div>
            <div style="font-size: 11px; color: var(--text-muted); margin-top: 6px;">💡 A 6-digit verification code will be sent to this email.</div>
          </div>
        </div>

        <div class="settings-light-panel" style="border-radius: 8px; padding: 16px;">
          ...
          <div class="form-group" style="margin-bottom: 0;">
            ...
            <input class="form-control" id="emailOtpCode" placeholder="Enter 6-digit code from email" maxlength="6" style="text-align: center; font-size: 20px; letter-spacing: 6px; font-weight: 600;">
            <div style="font-size: 11px; color: var(--text-muted); margin-top: 6px; text-align: center;">
              Didn't receive? <a href="#" onclick="sendEmailOTP(); return false;" style="color: var(--info); text-decoration: none; font-weight: 600;">Resend Code</a>
            </div>
          </div>
        </div>

        <div style="display: flex; gap: 8px; margin-top: 16px;">
          <button class="btn-outline" onclick="cancel2FASetup()" style="flex: 1;">Cancel</button>
          <button class="btn-gold" onclick="verifyEmailOTP()" style="flex: 1;">✓ Enable 2FA</button>
        </div>
      </div>

      <!-- Enabled Panel -->
      <div id="twoFactorEnabled" style="display: <?php echo $is2faActive ? 'block' : 'none'; ?>; background: var(--success-bg); border: 1px solid rgba(46,125,50,0.2); border-radius: 8px; padding: 16px; margin-top: 12px;">
        <div style="display: flex; align-items: start; gap: 12px; margin-bottom: 12px;">
          <div style="width: 40px; height: 40px; border-radius: 50%; background: var(--success); color: white; display: flex; align-items: center; justify-content: center; font-size: 20px; flex-shrink: 0;">✓</div>
          <div style="flex: 1;">
            ...
            <div style="font-size: 12px; color: var(--text-mid); line-height: 1.5;">
              Codes sent to: <span id="active2FAEmail" style="font-weight: 600; font-family: monospace;"><?php echo htmlspecialchars($userEmail); ?></span>
            </div>
          </div>
        </div>

        <div class="settings-light-panel" style="border-radius: 8px; padding: 12px; margin-bottom: 12px; border: 1px solid var(--border);">
          ...
          <button class="btn-outline btn-sm" onclick="downloadRecoveryCodes()" style="width: 100%; margin-top: 8px;">⬇ Download Recovery Codes</button>
        </div>

        <div class="two-factor-actions">
          <button class="btn-outline btn-sm" onclick="change2FAEmail()" style="flex: 1;">✉ Change Email</button>
          <button class="btn-outline btn-sm" onclick="regenerateRecoveryCodes()" style="flex: 1;">🔄 Regenerate Codes</button>
          <button class="btn-danger btn-sm" onclick="disable2FA()">✖ Disable 2FA</button>
        </div>
      </div>
    </div>
  </div>
