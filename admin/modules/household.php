...
<style>
  ...
  /* Printing handles by Master Print CSS in main.css */
...
<div class="sub-panel active" id="hh-list">
  <div class="sec-head">
    ...
    <div class="sec-head-actions">
      <button class="btn btn-gold" onclick="try{ openNewHHForm(); }catch(e){ alert('Error: '+e); }">✚ Add
        Household</button>
    </div>
  </div>
  <div class="filter-bar">
    <input class="filter-input" placeholder="🔍 Search household head..." id="hhSearch" oninput="applyHHFilters()"
      style="flex:1">
...
<!-- Form Panel (RBI Form A) -->
<div class="sub-panel" id="hh-form">
  <div class="breadcrumb">
    <a onclick="showHHSubPanel('hh-list')">Household</a> › <span id="hhFormBreadcrumb">RBI Form A Record</span>
  </div>
  <div class="sec-head">
    <div class="sec-head-left">
    ...
    <div class="sec-head-actions">
      <button class="btn btn-outline btn-sm" onclick="showHHSubPanel('hh-list')">← Back</button>
      <div
        style="display:flex; align-items:center; gap:8px; background:var(--slate); padding:4px 10px; border-radius:6px; border:1px solid var(--border);">
        <span style="font-size:11px; font-weight:600; color:var(--text-muted);">PUROK:</span>
        <select class="filter-select" id="hhPurokInput"
          style="padding:2px 5px; font-size:12px; border:none; background:transparent;">
          <option>GEMELINA</option>
          <option>NARRA</option>
          <option>YAKAL</option>
          <option>ALMASIGA</option>
      ...
      <button class="btn btn-primary btn-sm"
        onclick="const oldT=document.title; document.title=' '; window.print(); document.title=oldT;">🖨 Print RBI Form
        A</button>
  ...
  <div class="hhr-paper">
    ...
    <div class="hhr-meta-section">
      ...
      <div class="hhr-meta-item"><span>HOUSEHOLD ADDRESS :</span>
        <div class="hhr-meta-line">
          <div contenteditable="true" class="hhr-input-clean" id="hhAddressInput"
            style="text-align:left; font-family:Arial; font-size:11px;"></div>
        </div>
      </div>
      <div class="hhr-meta-item"><span>NO. OF HOUSEHOLD MEMBERS :</span>
        <div class="hhr-meta-line"><input class="hhr-input-clean" id="hhMemberCount" readonly
            style="width:40px; font-family:Arial; font-size:11px;"></div>
      </div>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th rowspan="2" style="width: 20%; font-size: 8px; font-weight: normal; text-transform: none;">
              Indicate if Labor/employed, Unemployed, PWD, OFW, Solo Parent,<br> Out of School Youth (OSY), Out of
              School
    ...
    <div class="hhr-disclaimer-paper">
      I hereby certify that the above information are true and correct to the best of my knowledge. I understand that
      ...
      planning, and as an updated reference in the number of inhabitants of the Barangay. Therefore, I grant my consent
      and recognize the authority of the Barangay to process my personal information, subject to the provision of the
...
<script>
(function startAdminHouseholdRealtime(intervalMs = 1000) {
  if (window._adminHouseholdRealtimeStarted) return;
  window._adminHouseholdRealtimeStarted = true;
  ...
  function shouldRefresh() {
    return document.getElementById('hh-list')?.classList.contains('active');
  }

  window._adminHouseholdRealtimePoll = setInterval(async () => {
    try {
      if (shouldRefresh()) await refreshHouseholds();
    } catch (e) {
      console.warn('Admin household realtime poll error', e);
    }
  }, intervalMs);

  window.stopAdminHouseholdRealtime = function() {
    if (window._adminHouseholdRealtimePoll) clearInterval(window._adminHouseholdRealtimePoll);
    window._adminHouseholdRealtimeStarted = false;
  };
})();
</script>
