...
<!-- full residents.html content inserted below -->
<style>
  /* ══════════════════ RESIDENTS MODULE STYLES ══════════════════ */
  ...
  /* ══════════════════ INDIVIDUAL RECORD FORM (RBI Form B) ══════════════════ */
  .ird-paper {
    background: #fff;
  ...
  .ird-header-top .form-id {
    font-size: 14px;
    font-weight: bold;
  }

  .ird-header-top .form-title {
    font-size: 16px;
    font-weight: bold;
    text-align: center;
    flex: 1;
  ...
  .ird-grid-5 {
    grid-template-columns: 1fr 1.5fr 0.4fr 1fr 1.2fr;
  }
...
<div class="sub-panel active" id="res-list">
  <div class="sec-head">
    ...
    <div class="sec-head-actions">
      <button class="btn btn-primary" onclick="openNewResidentForm()">📄 Individual Record</button>
    </div>
  </div>
  <div class="filter-bar">
    <input class="filter-input" placeholder="🔍 Search name or address..." id="resSearch"
      oninput="filterResidents(this.value)" style="flex:1;min-width:200px">
...
<div class="sub-panel" id="res-individual">
  ...
  <!-- IRD FORM (RBI Form B Revised 2024) -->
  <div class="ird-paper">
    ...
    <div class="ird-disclaimer">
      I hereby certify that the above information is true and correct to the best of my knowledge. I understand that for
      the Barangay to carry out its mandate pursuant to Section 394 (d)(6) of the Local Government Code of 1991, they
      must necessarily process my personal information for easy identification of inhabitants, as a tool in planning,
      and as an updated reference in the number of inhabitants of the Barangay. Therefore, I grant my consent and
      recognize the authority of the Barangay to process my personal information, subject to the provision of the
...
<script>
(function startAdminResidentsRealtime(intervalMs = 1000) {
  if (window._adminResidentsRealtimeStarted) return;
  window._adminResidentsRealtimeStarted = true;
  ...
  function shouldRefresh() {
    ...
    const individualPanel = document.getElementById('res-individual');
    return listPanel?.classList.contains('active') && !individualPanel?.classList.contains('active');
  }

  window._adminResidentsRealtimePoll = setInterval(async () => {
    try {
      if (shouldRefresh()) await refreshResidents();
    } catch (e) {
      console.warn('Admin residents realtime poll error', e);
    }
  }, intervalMs);

  window.stopAdminResidentsRealtime = function() {
    if (window._adminResidentsRealtimePoll) clearInterval(window._adminResidentsRealtimePoll);
    window._adminResidentsRealtimeStarted = false;
  };
})();
</script>
