...
<style>
  .cert-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; margin-bottom: 20px; }
  .cert-card { background: var(--white); border: 1px solid var(--border); border-radius: var(--radius); padding: 14px 16px; display: flex; align-items: center; gap: 12px; box-shadow: var(--shadow); cursor: pointer; }
  .cert-card:hover { background: var(--slate); }
  .cert-icon { width: 40px; height: 40px; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0; }
  .cert-count { font-family: 'Inter', sans-serif; font-size: 24px; font-weight: 700; line-height: 1; }
  .cert-label { font-size: 12px; color: var(--text-muted); font-weight: 500; }
...
<div class="sec-head">
  <div class="sec-head-left">
    ...
    <p>View and track issued certificates</p>
  </div>
  <!-- Clerk: NO Add Certificate button -->
</div>
...
<!-- Certificate View Modal (Clerk can view but not issue new) -->
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div class="form-group"><label class="form-label">Preview</label>
        <div id="cPreview" class="c-preview">
    ...
    <div class="modal-footer">
      <button class="btn btn-outline" onclick="closeModal('modal-cert')">Close</button>
...
<!-- Indigency Modal (view only for clerk) -->
  ...
  <div class="modal modal-lg">
    ...
    <div class="modal-body">
      <div class="form-row" style="position:relative;">
        <div class="form-group" style="flex:1;min-width:0;"><label class="form-label">Applicant's Full Name</label><input class="form-input" id="indName" placeholder="Full name" autocomplete="off" oninput="adminSearchResidents(this.value)"></div>
        ...
        <div class="form-group"><label class="form-label">Civil Status</label><input class="form-input" id="indCivil" placeholder="e.g. Single, Married"></div>
      </div>
      <div class="form-group"><label class="form-label">Street Address</label><input class="form-input" id="indAddress" placeholder="House no., Street"></div>
      <div class="form-group"><label class="form-label">Purpose</label><input class="form-input" id="indPurpose" placeholder="e.g. medical financial assistance"></div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Control No.</label><input class="form-input" id="indControlNo" placeholder=""></div>
        <div class="form-group"><label class="form-label">Date</label><input class="form-input" type="date" id="indDate"></div>
      </div>
      <hr>
      <div class="form-group"><label class="form-label">Preview</label>
    ...
    <div class="modal-footer">
      <button class="btn btn-outline" onclick="closeModal('modal-indigency')">Close</button>
...
function updateCertificateDate() {
  ...
  if (!dateInput) { const today = new Date(); dateInput = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`; if (cDateEl) cDateEl.value = dateInput; }
  const date = new Date(dateInput + 'T00:00:00');
  const monthNames = ['January','February','March','April','May','June','July','August','September','October','November','December'];
  const previewDay = document.getElementById('cPreviewIssuedDay'); const previewMonthYear = document.getElementById('cPreviewIssuedMonthYear');
  if (previewDay) previewDay.textContent = date.getDate();
  if (previewMonthYear) previewMonthYear.textContent = `${monthNames[date.getMonth()]}, ${date.getFullYear()}`;
  const hidDay = document.getElementById('cIssuedDay'); const hidMonth = document.getElementById('cIssuedMonth'); const hidYear = document.getElementById('cIssuedYear');
  if (hidDay) hidDay.value = date.getDate(); if (hidMonth) hidMonth.value = monthNames[date.getMonth()]; if (hidYear) hidYear.value = date.getFullYear();
}

function updateResidencyDate() {
  const rDateEl = document.getElementById('rDate');
  let dateInput = rDateEl ? rDateEl.value : '';
  if (!dateInput) { const today = new Date(); dateInput = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`; if (rDateEl) rDateEl.value = dateInput; }
  const date = new Date(dateInput + 'T00:00:00');
  const monthNames = ['January','February','March','April','May','June','July','August','September','October','November','December'];
  const previewDay = document.getElementById('rPreviewIssuedDay'); const previewMonthYear = document.getElementById('rPreviewIssuedMonthYear');
  if (previewDay) previewDay.textContent = date.getDate();
  if (previewMonthYear) previewMonthYear.textContent = `${monthNames[date.getMonth()]}, ${date.getFullYear()}`;
  const hidDay = document.getElementById('rIssuedDay'); const hidMonth = document.getElementById('rIssuedMonth'); const hidYear = document.getElementById('rIssuedYear');
  if (hidDay) hidDay.value = date.getDate(); if (hidMonth) hidMonth.value = monthNames[date.getMonth()]; if (hidYear) hidYear.value = date.getFullYear();
}

document.addEventListener('DOMContentLoaded', function() { updateCertificateDate(); updateResidencyDate(); });
</script>
